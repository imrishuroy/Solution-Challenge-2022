# POTENSHEIA
<img width="600" align='left' src="https://user-images.githubusercontent.com/72685035/161909863-6cb34cad-b599-4e92-b0e4-a4a21c61df94.png">


**INTRODUCTION : Unleah your inner potential to a unicorn career👩‍💻**


Right from childhood women face lot of struggles right from the time they are born, in rural areas people do not even consider educating or sending their daughters to higher studies, WHYYYYY???
See the potential every woman has in her, just be her support and witness the wonders she would do. 

“Human’s rights are women's rights and women's rights are human’s rights”.Let's make this world a better and more equal place for every individual. 

# [Potensheia App available on PlayStore](https://play.google.com/store/apps/details?id=com.exordium.addictx "Link to App") :iphone:

## Right from childhood women face lot of struggles right from the time they are born, in rural areas people do not even consider educating or sending their daughters to higher studies, WHYYYYY??? See the potential every woman has in her, just be her support and witness the wonders she would do.
- - - -
## Overview
##### Duration : 2021.03.04 ~ 2021.07.01 ⏰
- - - -
### Team member 💁🏻‍♀️💁🏻
#### Developer : Ruchika Suryawanshi, Ruchi Palkhe, Rishi Kumar, Nishtha Sarawat 

- - - -
### Technology of Use 💻
####  Tools:-
##### 1) Android Studio
##### 2) Adobe XD

#### Google Technologies:-
##### 1) Firebase
##### 2) Cloud Firestore
##### 3) Cloud Storage
##### 4) Cloud Functions
##### 5) Cloud Messaging
##### 6) Google Cloud Platform(Dialog Flow)
##### 7) Authentication
##### 8) Play Console
##### 9) Google Forms for taking surveys
##### 10) Google Sheets for in app surveys
##### 11) Google Translator
##### 12) Google ML Kit
##### 13) Google Teachable Machine
##### 14) Google Maps

#### Languages:-
##### 1) Flutter
##### 2) Dart
##### 3) JavaScript
- - -
### Main Features
##### 1) Check Your Addiction Level
###### With Addict-X personalized plans, you can check your level of addiction, visualize the pattern in your addiction, and focus on dealing with addiction.


##### 2) Guided Therapy
###### Guided Therapy to make you feel better, It can be quite a pleasurable experience to step outside of the present moment, and can be like a much needed mini break from reality.

##### 3) Examine the triggers
###### Recap each day and look for patterns that made it easier or more difficult than the day before.

##### 4) Be a part of Emerging Community
###### Connect with the people who facing same problems . When you're trapped in an addiction cycle, recovery will seem impossible. Change is possible with the right therapy, coping strategies, and support from Addict-X App, no matter how helpless you feel.

##### 5) Connect to the Experts
###### You can communicate with the expert and seek advice from them through chat or by scheduling an appointment.

##### 6) Yoga Plans
###### Yoga plans to cure your addiction and its side effects.

##### 7) Diet Plans
###### Diet plans to cure your addiction and its side effects.

##### 8) Multi-Language Support
###### AddictX contains 9 different languages to choose from.
- - - 
### Features 💡
##### 1) Habit Building
##### 2) Audio Therapy
##### 3) Todo
##### 4) Social Feed
##### 5) Post a query
##### 6) Connect to Experts
##### 7) Plans(Daily, Yoga, Diet, Audio)
##### 8) Resources
##### 9) Bot
##### 10) Chat feature
##### 11) Questionnaire
##### 12) Select Avatar
##### 13) Daily Updates with notification.
##### 14) Direct Google Login.
##### 15) Features Accesible even without login.
##### 16) Multi-Language Support
##### 17) AI driven mood detector.
- - -
### Steps To Use Run This Project:
##### 1) Install Flutter.
##### 2) Install Dart.
##### 3) Clone this repo to desired location.
##### 4) Run Pub Get and Pub Upgrade.
##### 5) Run this Project on your android emulator or connect real device to check this out.
- - -
